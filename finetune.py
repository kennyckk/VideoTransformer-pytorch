'''this file is to execute finetuning of custom datasets of Dog Data'''
...
def parse_args():
	...
	parser.add_argument(
		'-objective', type=str, default='supervised', # change to supervised as default
	...
	parser.add_argument(
		'-root_dir', type=str, required=True, default=os.getcwd(),
		help='the path to root dir for work space')

	# Data
	parser.add_argument(
		'-num_class', type=int, required=True,default=2, #set to 2 class for dog emotions
		help='the num class of dataset used')
	parser.add_argument(
		'-num_samples_per_cls', type=int, default=120, #set to 120 per cls
		help='the num samples of per class')
	parser.add_argument(
		'-img_size', type=int, default=224, #strictly comply
		help='the size of processed image')
	parser.add_argument(
		'-num_frames', type=int, required=True,default=16,#strictly comply
		help='the mumber of frame sampling')
	parser.add_argument(
		'-frame_interval', type=int, required=True,default=16, #strictly comply
	...
	# Model
	parser.add_argument(
		'-arch', type=str, default='vivit', #changed to vivit
		help='the choosen model arch from [timesformer, vivit]')
	parser.add_argument(
		'-attention_type', type=str, default='fact_encoder', #changed to model 2
		help='the choosen attention type using in model')
	parser.add_argument(
		'-pretrain_pth', type=str, required=True,default=os.path.join(os.getcwd(),'vivit_model.pth'), #change to default first
	...
	parser.add_argument(
		"-warmup_epochs", default=1, type=int,
